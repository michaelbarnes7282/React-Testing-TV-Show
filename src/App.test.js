...
import { rendor, waitFor, render } from '@testing-library/react';
...
test('render main page and click on season dropdown', async () => {
    await mockFetchShow.mockResolvedValueOnce(showData);
    const { getByText } = render(<App />);
    await waitFor(() => expect(getByText(/select a season/i)).toBeInTheDocument())
    const dropdown = getByText(/select a season/i)
    userEvent.click(dropdown)
    expect(dropdown.parentElement.parentElement).toHaveClass('is-open')
})